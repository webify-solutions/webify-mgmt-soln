<?php
namespace App\Test\TestCase\Model\Table;

use App\Model\Table\InvoiceItemTable;
use Cake\ORM\TableRegistry;
use Cake\TestSuite\TestCase;

/**
 * App\Model\Table\InvoiceItemTable Test Case
 */
class InvoiceItemTableTest extends TestCase
{

    /**
     * Test subject
     *
     * @var \App\Model\Table\InvoiceItemTable
     */
    public $InvoiceItem;

    /**
     * Fixtures
     *
     * @var array
     */
    public $fixtures = [
        'app.invoice_item',
        'app.invoice',
        'app.order_item',
        'app.product',
        'app.organization'
    ];

    /**
     * setUp method
     *
     * @return void
     */
    public function setUp()
    {
        parent::setUp();
        $config = TableRegistry::getTableLocator()->exists('InvoiceItem') ? [] : ['className' => InvoiceItemTable::class];
        $this->InvoiceItem = TableRegistry::getTableLocator()->get('InvoiceItem', $config);
    }

    /**
     * tearDown method
     *
     * @return void
     */
    public function tearDown()
    {
        unset($this->InvoiceItem);

        parent::tearDown();
    }

    /**
     * Test initialize method
     *
     * @return void
     */
    public function testInitialize()
    {
        $this->markTestIncomplete('Not implemented yet.');
    }

    /**
     * Test validationDefault method
     *
     * @return void
     */
    public function testValidationDefault()
    {
        $this->markTestIncomplete('Not implemented yet.');
    }

    /**
     * Test buildRules method
     *
     * @return void
     */
    public function testBuildRules()
    {
        $this->markTestIncomplete('Not implemented yet.');
    }
}
